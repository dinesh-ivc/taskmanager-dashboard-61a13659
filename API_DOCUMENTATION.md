...
The Swagger specification is pre-generated in `public/swagger.json`. After adding or modifying API documentation:

1. Run `npm run generate-swagger` to regenerate the spec
2. The updated spec will be written to `public/swagger.json`
3. Refresh `/api-docs` in your browser to see the changes

**Note**: You don't need to regenerate the spec for every change during development. The spec is loaded at runtime from `public/swagger.json`.

## Configuration

The Swagger configuration is located in `lib/swagger-script.js`. You can modify:
